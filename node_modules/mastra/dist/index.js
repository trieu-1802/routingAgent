#! /usr/bin/env node
import { COMPONENTS, parseComponents, LLMProvider, parseLlmProvider, parseMcp, create, checkForPkgJson, checkAndInstallCoreDeps, interactivePrompt, init, logger, FileService, createLogger, shouldSkipDotenvLoading, DepsService } from './chunk-H53A6HJD.js';
export { create } from './chunk-H53A6HJD.js';
...
var DevLogger = class {
  ...
  ready(host, port, startTime, https) {
    ...
    console.info("");
    console.info(`${pc2.dim("\u2502")} ${pc2.bold("Studio:")} ${pc2.cyan(`${protocol}://${host}:${port}`)}`);
    console.info(`${pc2.dim("\u2502")} ${pc2.bold("API:")}    ${`${protocol}://${host}:${port}/api`}`);
    console.info("");
    console.info(pc2.inverse(pc2.yellowBright(" Important ")) + pc2.yellowBright(" Mastra v1.0.0 will go stable soon."));
    console.info(
      pc2.yellowBright(
        "Please update to the latest v0.x and try out the v1.0.0-beta releases! Follow the migration guide:"
      )
    );
    console.info(pc2.yellowBright("https://mastra.ai/guides/v1/migrations/upgrade-to-v1/overview"));
  }
  bundling() {
  ...
  try {
    ...
    if (currentServerProcess.stdout) {
      currentServerProcess.stdout.on("data", (data) => {
        ...
        if (!output.includes("Studio available") && !output.includes("\u{1F468}\u200D\u{1F4BB}") && !output.includes("Mastra API running on port")) {
