...
async function validateAndSaveScore(storage, payload) {
  const payloadToSave = chunkW54LWXCG_cjs.saveScorePayloadSchema.parse(payload);
...
_Mastra_decorators = [chunk7GUQ2EQ4_cjs.InstrumentClass({
  prefix: "mastra",
...
exports.Mastra = class Mastra {
  ...
  constructor(config) {
    ...
    const workflowEventProcessor = new chunkQLPSR67Y_cjs.WorkflowEventProcessor({
      mastra: this
    });
    ...
    this.#telemetry = chunk7GUQ2EQ4_cjs.Telemetry.init(config?.telemetry);
    ...
    if (config?.observability) {
      chunkQGJF6TRY_cjs.setupAITracing(config.observability);
    }
...
`
  ...
  registerAITracingExporters() {
    const allTracingInstances = chunkQGJF6TRY_cjs.getAllAITracing();
  ...
  setTelemetry(telemetry) {
    this.#telemetry = chunk7GUQ2EQ4_cjs.Telemetry.init(telemetry);
    if (this.#agents) {
      Object.keys(this.#agents).forEach(key => {
        if (this.#telemetry) {
   ...
   */
  async shutdown() {
    await chunkQGJF6TRY_cjs.shutdownAITracingRegistry();
...
//# sourceMappingURL=chunk-MSFEDB5Z.cjs.map
//# sourceMappingURL=chunk-MSFEDB5Z.cjs.map