import { DefaultVoice } from './chunk-GEHKQTB2.js';
import { STREAM_FORMAT_SYMBOL, EMITTER_SYMBOL } from './chunk-NLNKQD2T.js';
import { InstrumentClass, Telemetry } from './chunk-5S5X3QPR.js';
import { MessageList, DefaultGeneratedFile, DefaultGeneratedFileWithType } from './chunk-NFJJ6B6P.js';
import { resolveModelConfig } from './chunk-72F4RGK7.js';
import { MastraLLMV1 } from './chunk-ZZIYFEVM.js';
import { executeHook } from './chunk-TTELJD4F.js';
import { zodToJsonSchema } from './chunk-PJKCPRYF.js';
import { removeUndefinedValues, wrapMastra, selectFields, getOrCreateSpan, getValidTraceId, ensureToolProperties, makeCoreTool, createMastraProxy, isZodType, ModelSpanTracker, delay } from './chunk-BBE22VBK.js';
...
function createStreamStep({capabilities,runId,returnScorerData,format="mastra",requireToolApproval,resumeContext,agentId,toolCallId,methodType,saveQueueManager,memoryConfig,memory,resourceId}){return createStep({id:"stream-text-step",inputSchema:z.any(),// tried to type this in various ways but it's too complex
...
${z42.prettifyError(result.error)}
`,details:{value:typeof value==="object"?JSON.stringify(value):String(value)}},result.error)};}}catch(error){return {success:false,error:error instanceof Error?error:new Error("Zod validation failed",{cause:error})};}}try{if(typeof this.schema==="object"&&!this.schema.jsonSchema){const result=await safeValidateTypes({value,schema:jsonSchema(this.schema)});return result;}else if(this.schema.jsonSchema){const result=await safeValidateTypes({value,schema:this.schema});return result;}else {return {success:true,value};}}catch(error){return {success:false,error:error instanceof Error?error:new Error("Validation failed",{cause:error})};}}/**
...
//# sourceMappingURL=chunk-GSQD3QNR.js.map
//# sourceMappingURL=chunk-GSQD3QNR.js.map